...
class TransactionBanquePage extends StatefulWidget {
  final Banque banque;
  const TransactionBanquePage({Key? key, required this.banque}) : super(key: key);

  @override
  State<TransactionBanquePage> createState() => _TransactionBanquePageState();
}

class _TransactionBanquePageState extends State<TransactionBanquePage> {

  List<TransactionU> allTransactions = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myDefaultAppBar('Transactions (${widget.banque.libelle!})', context),
                    ...
                    child: DataTable(
                      ...
                      rows: allTransactions.map((transaction) {
                        return DataRow(cells: [
                          ...
                          DataCell(Text("${NumberFormat('#,###', 'fr_FR').format(transaction.montant!)} ${CnxInfo.symboleMonnaie}")),
                              ...
                              builder: (ctx) => Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    textMontserrat("${NumberFormat('#,###', 'fr_FR').format(transaction.montant!)} ${CnxInfo.symboleMonnaie}", 16, Colors.black, TextAlign.start,),
                                    textMontserrat("Mode règlement:", 16, Colors.black, TextAlign.start, fontWeight: FontWeight.w700),
                                    textMontserrat(transaction.mode_reglement!, 16, Colors.black, TextAlign.start,),
                                    ...
                                    transaction.categorie_depense != null ? textMontserrat(transaction.categorie_depense!, 16, Colors.black, TextAlign.start) : SizedBox(),
                                  ],
                                ),
                              ),
                            );
  ...
  Future<void> getAllTransactions() async {
    var request = http.Request('GET', Uri.parse("${r_transactionsBanque}/${widget.banque.id.toString()}"));
    ...
    //print(responseData['data']['transactions']);

    if (response.statusCode == 200) {
      allTransactions.clear();
      late TransactionU transaction;
      for(int i=0; i < responseData['data']['transactions'].length; i++){
        transaction = TransactionU.fromJson(responseData['data']['transactions'][i]);
