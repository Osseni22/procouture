...
class NouvelleDepenseSavePage extends StatefulWidget {
  /*final int typeTransaction;*/ // 1 Pour recette, 2 pour dépense
  final int base; // 1 for Confection, 2 for Boutique, 3 for Banque
  const NouvelleDepenseSavePage({Key? key,/* required this.typeTransaction*/ required this.base}) : super(key: key);

  @override
  State<NouvelleDepenseSavePage> createState() => _NouvelleDepenseSavePageState();
}

class _NouvelleDepenseSavePageState extends State<NouvelleDepenseSavePage> {
  ...
  // Create a 'recette'
  Future<void> createNouvelleDepense() async {
    ...
    var request = http.MultipartRequest('POST', Uri.parse(r_tresorerieNewDepense));
  ...
  String getPageTitle(){
    if(widget.base == 1){
      return 'Nouvelle dépense confection';
    }
    if(widget.base == 2){
      return 'Nouvelle dépense boutique';
    }
    if(widget.base == 3){
      return 'Nouvelle dépense banque';
    }
