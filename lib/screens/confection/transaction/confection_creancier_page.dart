...
class ConfectionCreancierPage extends StatefulWidget {
  const ConfectionCreancierPage({Key? key}) : super(key: key);

  @override
  State<ConfectionCreancierPage> createState() => _ConfectionCreancierPageState();
}

class _ConfectionCreancierPageState extends State<ConfectionCreancierPage> {
                  ...
                  child: DataTable(
                      ...
                      rows: allCreanciers.map((data) => DataRow(
                          cells: [
                            ...
                            DataCell(Text("${NumberFormat('#,###', 'fr_FR').format(data['montant_creance'])} ${CnxInfo.symboleMonnaie}")),
                          ])
                      ).toList()
                  ),
                ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Container(alignment: Alignment.centerLeft, child: textOpenSans("${NumberFormat('#,###', 'fr_FR').format(totalCreance)} ${CnxInfo.symboleMonnaie}", 16, Colors.black, TextAlign.center,fontWeight: FontWeight.bold)),
                    ],
                  ),
                ]
            ),
  ...
  Future<void> getCreanciers() async {
    var request = http.Request('GET', Uri.parse(r_confectionsCreanciers));
